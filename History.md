...
- 26-03-09 (26.3.6+13.1):
  - add move_file() 
- 26-03-09 (26.3.5+9.1):
  - read_dir_deepが1階層多くreadしていたので修正
- 26-03-04 (26.3.4+4.3):
  - read_dirをwindowsの場合osから取得するようにしてみた
    - 自身を取得していたり、更新時間がnsになっていたのを timestampに修正
- 26-03-03 (26.3.1+3.1):
  - is_movieに .webm を追加

## 26-02

- 26-02-23 (26.2.23+23.2):
  - ex系を utilに変更
  - zip_ex での オープン処理で dir補完が最終の / のみで判定されていたので、複数階層に渡って分解して補完するように修正
- 26-02-18 (26.2.21+21.1):
  - Zipをロードした時、dir情報が含まれていないことが合ったので補完するように対応
- 26-02-18 (26.2.19+18.2):
  - ZipINfo を str 及び pathから変換する際に zipを考慮するように対応
  - ZipInfo をpubに
- 26-02-15 (26.2.10+15.1 - 26.2.17+15.8):
  - ZipExで、zip系の関数をまとめた
    - zip内の dirの終端の / を削除するように対応
  - files/applicationで、zip系の関数を追加
  - FileInfoにzip_infoを追加
  - FileInfo作成時、最後の/を削除
  - FileInfoに in_zipを追加
- 26-02-14 (26.2.5+14.1 - 9+14.5):
  - PathEx を追加し、パス変換時 linux系 の区切り文字を利用するように対応
    - OsStr系を to_string_ex() で変換できるように対応
  - FileInfo取得の際に、metaをloadしない方法を追加
  - FileInfo に is_zip を追加
